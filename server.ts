...
async function startServer() {
  ...
  app.delete("/api/logs/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      await db.delete(logs).where(eq(logs.id, req.params.id));
  ...
  app.delete("/api/participants/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      ...
      // 1. Reassign created records to the deleting admin to preserve integrity
      await db.update(transactions).set({ userId: adminId }).where(eq(transactions.userId, targetId));
      await db.update(tasks).set({ userId: adminId }).where(eq(tasks.userId, targetId));
      await db.update(events).set({ userId: adminId }).where(eq(events.userId, targetId));
      await db.update(attendanceSessions).set({ createdBy: adminId }).where(eq(attendanceSessions.createdBy, targetId));

      // 2. Clear assignment / user relation references
      await db.update(tasks).set({ assigneeId: null }).where(eq(tasks.assigneeId, targetId));
      await db.update(attendanceRecords).set({ userId: null }).where(eq(attendanceRecords.userId, targetId));

      // 3. Delete logs linked to the deleted user
      await db.delete(transactionLogs).where(eq(transactionLogs.userId, targetId));
      await db.delete(logs).where(eq(logs.userId, targetId));

      // 4. Finally delete the user from database
      await db.delete(users).where(eq(users.id, targetId));
      await logActivity(adminId, "Menghapus Peserta", `Menghapus peserta dengan ID ${targetId}`);

      res.json({ success: true });
    } catch (e: any) {
      console.error(e);
      res.status(400).json({ error: "Peserta gagal dihapus karena kesalahan database: " + e.message });
    }
  });
  ...
  app.delete("/api/transactions/:id", requireAuth, async (req: AuthRequest, res) => {
    try {
      await db.delete(transactionLogs).where(eq(transactionLogs.transactionId, req.params.id));
      await db.delete(transactions).where(eq(transactions.id, req.params.id));
      await logActivity(req.user!.id, "Hapus Keuangan", `Menghapus transaksi ID: ${req.params.id}`);
      res.json({ success: true });
    } catch (e) {
  ...
  app.delete("/api/transactions", requireAuth, async (req: AuthRequest, res) => {
    try {
      await db.delete(transactionLogs);
      await db.delete(transactions);
      await logActivity(req.user!.id, "Clear Keuangan", "Mengosongkan seluruh data transaksi keuangan");
      res.json({ success: true });
    } catch (e) {
