...
function AppContent() {
  ...
  const userPerms = React.useMemo(() => {
    ...
    // Auto-grant sidebar tab visibility for respective roles
    if (user.role === 'Bendahara') {
      basePerms.finance = basePerms.finance && basePerms.finance !== 'none' ? basePerms.finance : 'crud';
    }
    if (user.role === 'Sekretaris') {
      basePerms.calendar = basePerms.calendar && basePerms.calendar !== 'none' ? basePerms.calendar : 'crud';
      basePerms.attendance = basePerms.attendance && basePerms.attendance !== 'none' ? basePerms.attendance : 'crud';
      basePerms.tasks = basePerms.tasks && basePerms.tasks !== 'none' ? basePerms.tasks : 'crud';
    }
  ...
  const navItems = [
    ...
    ...(user?.nim === '223125416' || user?.nim === '223140101' ? [
    ...
    <div className="min-h-screen bg-gray-50 flex font-sans text-gray-900">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 z-50 flex flex-col transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-16 flex items-center px-6 border-b border-gray-100 flex-shrink-0">
          ...
          <h1 className="text-lg font-bold text-gray-900 tracking-tight">KKN Wotgalih Manager</h1>
          ...
          <div className="max-w-6xl mx-auto">
            {dataLoading ? (
              ...
              <Routes>
                ...
                {(user?.nim === '223125416' || user?.nim === '223140101') && <Route path="/backup-restore" element={<BackupRestoreView getToken={getToken} />} />}
                <Route path="*" element={<Navigate to="/dashboard" replace />} />
              </Routes>
            )}
          </div>
