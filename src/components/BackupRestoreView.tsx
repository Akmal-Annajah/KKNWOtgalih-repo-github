  ...
  const processBackupExcel = (file: File) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const hasAdmin = usersList.some((u: any) => String(u.nim || '').trim() === '223125416' || String(u.nim || '').trim() === '223140101');
        if (!hasAdmin) {
          setRestoreError("File backup tidak mengandung akun Admin utama dengan NIM 223125416 atau 223140101. Restore dilarang demi mencegah lock-out.");
          return;
        }
